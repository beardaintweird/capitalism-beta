    ...
    renderHands(players, turn){
      for(let i = 0, x = this.players.length; i < x; i++){
        ...
        if($(`#p${this.players[i].number}-hand`).length){
          $(`#p${this.players[i].number}-hand`).children().remove();
        } else {
          $(`#p${this.players[i].number}`).append(`<div class="hand" id="p${this.players[i].number}-hand"></div>`)
        }
        for(let j = 0, y = this.players[i].hand.cards.length; j < y; j++){
          $(`#p${this.players[i].number}-hand`).append(`
            <button value = "${this.players[i].hand.cards[j].rank}"
            class="player${this.players[i].number}-hand" id="${this.players[i].hand.cards[j].title}of${this.players[i].hand.cards[j].suit}">
              <img src='${this.players[i].hand.cards[j].image}'></img>
            </button>
            `);
        }
        this.renderDoublesTriplesQuads(players[i]);
        if(!$(`#player${this.players[i].number}-buttons`).length){
          $(`#p${this.players[i].number}`)
          .append(`<div class="playerHandButtons" id="player${this.players[i].number}-buttons"></div>`)
        }
        if(!$(`#player${this.players[i].number}-complete`).length){
          $(`#player${this.players[i].number}-buttons`).append(`<button id="player${this.players[i].number}-complete" class="completionBtn">Complete</button>`);
        }
        if(!$(`#player${this.players[i].number}-pass`).length){
          $(`#player${this.players[i].number}-buttons`).append(`<button class="passBtn" id="player${this.players[i].number}-pass">Pass</button>`);
        }
        // remove the complete and pass buttons from players who are done
        if(this.players[i].isDone){
          $(`#player${this.players[i].number}-pass`).remove();
          $(`#player${this.players[i].number}-complete`).remove();
        }
      }
    },
